 ...
 * @version 1.0.0
 */
abstract class AmyFramework_Options extends AmyFramework_Abstract {
	...
	public function element_after() {
		...
		$out	= (isset( $this->field['info'] )) ? '<p class="amy-text-desc">' . $this->field['info'] . '</p>' : '';
	...
	public function element_debug() {
		...
		if ( (isset( $this->field['debug'] ) && $this->field['debug'] === true) || (defined( 'AMY_OPTIONS_DEBUG' ) && AMY_OPTIONS_DEBUG) ) {
			$value	= $this->element_value();

			$out	.= '<pre>';
			$out	.= '<strong>' . __( 'CONFIG', 'amy-framework' ) . ':</strong>';
			$out	.= "\n";
			...
			$out	.= "\n\n";
			$out	.= '<strong>' . __( 'USAGE', 'amy-framework' ) . ':</strong>';
			$out	.= "\n";
			$out	.= (isset( $this->field['id'] )) ? "amy_get_option( '" . $this->field['id'] . "' );" : '';

			if ( ! empty( $value ) ) {
				$out	.= "\n\n";
				$out	.= '<strong>' . __( 'VALUE', 'amy-framework' ) . ':</strong>';
				$out	.= "\n";
				ob_start();
		...
		if ( (isset( $this->field['debug_light'] ) && $this->field['debug_light'] === true) || (defined( 'AMY_OPTIONS_DEBUG_LIGHT' )) && AMY_OPTIONS_DEBUG_LIGHT ) {
			$out	.= '<pre>';
			$out	.= '<strong>' . __( 'USAGE', 'amy-framework' ) . ':</strong>';
			$out	.= "\n";
			$out	.= isset( $this->field['id'] ) ? 'cs_get_option(\'' . $this->field['id'] . '\');' : '';
			$out	.= "\n";
			$out	.= '<strong>' . __( 'ID', 'amy-framework' ) . ':</strong>';
			$out	.= "\n";
	...
	public function element_get_error() {
		...
		if ( ! empty( $amy_errors ) ) {
			foreach ( $amy_errors as $key => $value ) {
				if ( isset( $this->field['id'] ) && $value['code'] == $this->field['id'] ) {
					$out	.= '<p class="amy-text-warning">' . $value['message'] . '</p>';
				}
			}
		}
	...
	public function element_help() {
		return (isset( $this->field['help'] )) ? '<span class="amy-help" data-title="' . $this->field['help'] . '"><span class="fa fa-question-circle"></span></span>' : '';
	}
		...
		if ( is_array( $this->multilang ) ) {
			...
			foreach ( $this->multilang['languages'] as $key => $val ) {
				...
				if ( $key != $this->multilang['current'] ) {
					...
					$class		= 'AmyFramework_Option_' . $this->field['type'];
			...
			$out	.= '</fieldset>';
			$out	.= '<p class="amy-text-desc">' . sprintf( __( 'You are editing language: ( <strong>%s</strong> )', 'amy-framework' ), $this->multilang['current'] ) . '</p>';
		}
	...
	public function element_multilang() {
		return (isset( $this->field['multilang'] )) ? amy_language_defaults() : false;
	}
}

// load all of fields
amy_load_option_fields();
