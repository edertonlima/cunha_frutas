...
class Amy_Demo_Importer_Settings extends Amy_Demo_Importer_Base {
	...
	public function __construct( $data = null ) {
		...
		$this->theme_options_name	= apply_filters( 'amy_demo_importer_get_theme_options_name', AMY_OPTION );
	...
	public function save() {
		...
		$current_settings	= array(
			...
			'amy_custom_sidebars'	=> get_option( 'amy_custom_sidebar' ),
		);
		...
		$current_settings	= apply_filters( 'amy_demo_importer_save_current_settings', $current_settings );
	...
	public function restore() {
		...
		update_option( 'amy_custom_sidebars', $this->history['amy_custom_sidebars'] );
		...
		do_action( 'amy_demo_importer_restore_settings', $this->history );
	...
	public function add() {
		...
		if ( isset( self::$data->custom_sidebars ) && count( self::$data->custom_sidebars ) ) {
			$custom_sidebars	= get_option( 'amy_custom_sidebars' );

			foreach ( self::$data->custom_sidebars as $key => $name ) {
				$custom_sidebars[ $key ]	= $name;
			}

			update_option( 'amy_custom_sidebars', $custom_sidebars );
		}
		...
		if ( isset( self::$data->sidebars ) && count( (self::$data->sidebars) ) ) {
			...
			foreach ( self::$data->sidebars as $sidebar => $widgets ) {
				foreach ( $widgets as $widget ) {
					...
					$widget_options	= apply_filters( 'amy_demo_importer_add_widget', $widget_options, $widget, $sidebar, $sidebars_widgets );

					update_option( 'widget_' . $widget->name, $widget_options );

					Amy_Demo_Importer_Map::instance()->save( 'widgets', $widget->name . '-' . $widget->id, $new_id, null );
				}
			}

			$sidebars_widgets	= apply_filters( 'amy_demo_importer_add_sidebars_widgets', $sidebars_widgets );

			update_option( 'sidebars_widgets', $sidebars_widgets );
		}

		do_action( 'amy_demo_importer_add_settings', self::$data );
	}
	...
	public function remap() {
		$map		= Amy_Demo_Importer_Map::instance();
		...
		if ( isset( self::$data->theme_options ) ) {
			$theme_options	= apply_filters( 'amy_demo_importer_remap_theme_options', amy_decode_string( self::$data->theme_options ), self::$data->site_url );
		...
		if ( isset( self::$data->nav_menu_locations ) && count( self::$data->nav_menu_locations ) ) {
			...
			$locations	= apply_filters( 'amy_demo_importer_remap_nav_menu_locations', $locations );
			...
			foreach ( self::$data->sidebars as $sidebar => $widgets ) {
				foreach ( $widgets as $widget ) {
					...
					if ( isset( $widget_ids[ $widget->name . '-' . $widget->id ] ) && isset( $widget_options[ $widget_ids[ $widget->name . '-' . $widget->id ] ] ) ) {
						...
						$widget_options	= apply_filters( 'amy_demo_importer_remap_widget', $widget_options, $widget );
		...
		do_action( 'amy_demo_importer_remap_settings', self::$data );
	}
}
