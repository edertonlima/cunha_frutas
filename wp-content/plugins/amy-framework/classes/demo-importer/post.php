...
class Amy_Demo_Importer_Post extends Amy_Demo_Importer_Base {
	...
	public function add_post( $post ) {
		...
		if ( in_array( $post_type, apply_filters( 'amy_demo_importer_get_post_type_unique', array( 'post', 'page', 'product' ) ) ) && ($new_id = post_exists( $post->title, $post->content )) ) {
			$new	= false;
		} else {
			$new	= true;
			...
			do_action( 'amy_demo_importer_add_post', $post, $new_id );
		}
	...
	public function remove() {
		$ids	= Amy_Demo_Importer_Map::instance()->get( 'posts' );

		foreach ( $ids as $id ) {
			if ( $id[1] ) {
				wp_delete_post( $id[0], true );

				do_action( 'amy_demo_importer_delete_post', $id[0] );
			}
		}
	}
		...
		if ( isset( self::$data->posts ) && is_array( self::$data->posts ) ) {
			...
			foreach ( self::$data->posts as $post ) {
				if ( isset( $post_ids[ $post->id ] ) && $post_ids[ $post->id ][0] && $post_ids[ $post->id ][1] ) {
					...
					do_action( 'amy_demo_importer_remap_post', $post, self::$data->site_url );
				}
			}
		}
	}
