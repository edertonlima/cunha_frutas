...
class Amy_Demo_Importer_Term extends Amy_Demo_Importer_Base {
	...
	public function add_term( $term ) {
		if ( $new_id = term_exists( $term->name, $term->taxonomy ) ) {
			...
			do_action( 'amy_demo_importer_add_term', $term, $new_term );
		}

		Amy_Demo_Importer_Map::instance()->save( 'terms', $term->id, $new_id, $new );
	}

	public function remove() {
		$ids	= Amy_Demo_Importer_Map::instance()->get( 'terms' );
		...
		foreach ( $ids as $id ) {
			if ( $id[1] ) {
				...
				do_action( 'amy_demo_importer_delete_term', $id[0] );
			}
		}
	}
	...
	public function remap() {
		if ( isset( self::$data->terms ) && is_array( self::$data->terms ) ) {
			...
			foreach ( self::$data->terms as $term ) {
				...
				do_action( 'amy_demo_importer_remap_term', $term );
			}
		}
	}
}
