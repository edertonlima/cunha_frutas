 ...
 * @version 1.0.0
 */
class AmyFramework_Taxonomy extends AmyFramework_Abstract {
	...
	public function __construct( $options ) {
		$this->options	= apply_filters( 'amy_taxonomy_options', $options );
	...
	public static function instance( $options = array() ) {
		if ( is_null( self::$instance ) && AMY_ACTIVE_TAXONOMY ) {
	...
	public function add_taxonomy_fields() {
		foreach ( $this->options as $option ) {
			...
			$get_taxonomy	= amy_get_var( 'taxonomy' );
	...
	public function render_taxonomy_form_fields( $term ) {
		...
		$amy_errors	= get_transient( 'amy-taxonomy-transient' );

		wp_nonce_field( 'amy-taxonomy', 'amy-taxonomy-nonce' );

		echo '<div class="amy-framework amy-taxonomy amy-taxonomy-' . $classname . '-fields">';
		...
		foreach ( $this->options as $option ) {
			if ( $taxonomy == $option['taxonomy'] ) {
				...
				foreach ( $option['fields'] as $field ) {
					...
					echo amy_add_element( $field, $elem_value, $option['id'] );
				}
			}
		}
	...
	public function save_taxonomy( $term_id ) {
		if ( wp_verify_nonce( amy_get_var( 'amy-taxonomy-nonce' ), 'amy-taxonomy' ) ) {
			$errors		= array();
			$taxonomy	= amy_get_var( 'taxonomy' );
			...
			foreach ( $this->options as $request_value ) {
				if ( $taxonomy == $request_value['taxonomy'] ) {
						...
						foreach ( $request_value['fields'] as $field ) {
							if ( isset( $field['type'] ) && isset( $field['id'] ) ) {
								...
								if ( has_filter( 'amy_sanitize_' . $sanitize_type ) ) {
									$request[ $field['id'] ]	= apply_filters( 'amy_sanitize_' . $sanitize_type, $field_value, $field, $request_value['fields'] );
								}

								// validate options
								if ( isset( $field['validate'] ) && has_filter( 'amy_validate_' . $field['validate'] ) ) {
									$validate	= apply_filters( 'amy_validate_' . $field['validate'], $field_value, $field, $request_value['fields'] );
					...
					$request	= apply_filters( 'amy_save_taxonomy', $request, $request_key, $term_id );
			...
			set_transient( 'amy-taxonomy-transient', $errors, 10 );
		}
	}
