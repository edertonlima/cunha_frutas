 ...
 * @version 1.0.0
 */
class AmyFramework_Customize extends AmyFramework_Abstract {
	...
	public function __construct( $options ) {
		$this->options	= apply_filters( 'amy_customize_options', $options );

		do_action( 'amy_customize_options_config', $this->options );
	...
	public static function instance( $options = array() ) {
		if ( is_null( self::$instance ) && AMY_ACTIVE_CUSTOMIZE ) {
	...
	public function customize_register( $wp_customize ) {
		...
		amy_locate_template( 'functions/customize.php' );
		do_action( 'amy_customize_register', $wp_customize );
	...
	public function add_section( $wp_customize, $value, $panel = false ) {
		...
		foreach ( $sections as $section ) {
			...
			foreach ( $section['settings'] as $setting ) {
				$setting_name	= AMY_CUSTOMIZE . '[' . $setting['name'] . ']';
				...
				$wp_customize->add_setting($setting_name,
					wp_parse_args($setting, array(
							...
							'sanitize_callback'	=> 'amy_sanitize_clean',
						)
					)
				);
				...
				$control_args	= wp_parse_args($setting['control'], array(
					'unique'		=> AMY_CUSTOMIZE,
					'section'		=> $section['name'],
					'settings'		=> $setting_name,
					'priority'		=> $setting_priority,
				));

				if ( $control_args['type'] == 'amy_field' ) {
