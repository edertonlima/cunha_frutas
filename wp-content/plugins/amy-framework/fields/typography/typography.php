 ...
 * @version 1.0.0
 */
class AmyFramework_Option_typography extends AmyFramework_Options {
	...
	public function output() {
		...
		$default_variants	= apply_filters('amy_websafe_fonts_variants', array(
			'regular',
			'italic',
			'700',
			'700italic',
		...
		$websafe_fonts	= apply_filters('amy_websafe_fonts', array(
			'Arial',
			'Arial Black',
			'Comic Sans MS',
			'Impact',
		...
		$google_json	= amy_get_google_fonts();
		...
		if ( is_object( $google_json ) ) {
			...
			echo '<label class="amy-typography-family">';
			echo '<select name="' . $this->element_name( '[family]' ) . '" class="' . $is_chosen . $chosen_rtl . 'amy-typo-family" data-atts="family"' . $this->element_attributes() . '>';

			do_action( 'amy_typography_family', $family_value, $this );

			echo '<optgroup label="' . __( 'Web Safe Fonts', 'amy-framework' ) . '">';
			...
			echo '<optgroup label="' . __( 'Google Fonts', 'amy-framework' ) . '">';
			...
			if ( ! empty( $is_variant ) ) {
				...
				echo '<label class="amy-typography-variant">';
				echo '<select name="' . $this->element_name( '[variant]' ) . '" class="' . $is_chosen . $chosen_rtl . 'amy-typo-variant" data-atts="variant">';
			...
			echo '<input type="text" name="' . $this->element_name( '[font]' ) . '" class="amy-typo-font hidden" data-atts="font" value="' . $value['font'] . '" />';
		} else {
			echo __( 'Error! Can not load json file.', 'amy-framework' );
		}
