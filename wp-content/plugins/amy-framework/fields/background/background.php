 ...
 * @version 1.0.0
 */
class AmyFramework_Option_background extends AmyFramework_Options {
	...
	public function output() {
		...
		$button_title	= (isset( $button_title )) ? $button_title : __( 'Upload', 'amy-framework' );
		$frame_title	= (isset( $frame_title )) ? $frame_title : __( 'Upload', 'amy-framework' );
		$insert_title	= (isset( $insert_title )) ? $insert_title : __( 'Use Image', 'amy-framework' );

		echo '<div class="amy-field-upload">';
		echo '<input type="text" name="' . $this->element_name( '[image]' ) . '" value="' . $this->value['image'] . '"' . $this->element_class() . $this->element_attributes() . '/>';
		echo '<a href="#" class="button amy-add" data-frame-title="' . $frame_title . '" data-upload-type="' . $upload_type . '" data-insert-title="' . $insert_title . '">' . $button_title . '</a>';
		echo '</div>';
		...
		echo amy_add_element(array(
			'pseudo'		=> true,
			'type'			=> 'select',
			'class'			=> isset( $this->field['chosen'] ) ? 'chosen' : '',
		...
		echo amy_add_element(array(
			'pseudo'		=> true,
			'type'			=> 'select',
			'class'			=> isset( $this->field['chosen'] ) && $this->field['chosen'] ? 'chosen' : '',
